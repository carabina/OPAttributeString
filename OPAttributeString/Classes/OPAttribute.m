...
- (OPAttributeAttachmentBlock)attachment {
    return ^OPAttribute *(UIImage *image, NSInteger index, CGRect rect, AttachmentAlignment alignment) {
        ...
        [self.string addAttribute:NSBaselineOffsetAttributeName value:[NSNumber numberWithFloat:deltaY] range:NSMakeRange(index, 1)];
        return self;
...
- (OPAttributeIntIntBlock)fromTo {
    return ^OPAttribute *(NSInteger from, NSInteger to) {
        [self.ranges removeAllObjects];
        from = (from < 0 || from > self.string.string.length) ? 0 : from;
        to = (to < 0 || to > self.string.string.length) ? self.string.string.length : to;
...
- (OPAttributeStringBlock)rangeOf {
    ...
    return ^OPAttribute *(NSString *rangeOf) {
        ...
        [weakSelf.ranges removeAllObjects];
        
        NSValue *value = [NSValue valueWithRange:[weakSelf rangeOfObject:rangeOf]];
        [weakSelf.ranges addObject:value];
...
- (OPAttributeOperationBlock)insert {
    return ^OPAttribute *(id insert, ...) {
        ...
        index = (index < 0 || index > self.string.string.length) ? (int)self.string.length : index == 0 ? 0 : index;
...
- (OPAttributeOperationBlock)append {
    return ^OPAttribute *(id insert, ...) {
        ...
        NSInteger index = self.string.length;
...
- (void)removeObjectWithArray:(NSArray *)array {
    ...
    [strArray enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        [weakSelf.string deleteCharactersInRange:[self rangeOfObject:obj]];
    }];
}
